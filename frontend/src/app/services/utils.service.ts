...
export class UtilsService {
  ...
  removeHtmlTagsAndExtraSpaces(inputString: string) {
    const stringWithoutBrTags = inputString.replace(/<br\s*\/?>/g, ' ');
    const stringWithoutExtraSpaces = stringWithoutBrTags.replace(/\s+/g, ' ').trim();
  
    return stringWithoutExtraSpaces;
  }

  convertTimestampToActualTimestampDate(timestamp: number): number {
    const dateToConvert = new Date(timestamp);
    const actualDate = new Date();
  
    dateToConvert.setFullYear(actualDate.getFullYear());
    dateToConvert.setMonth(actualDate.getMonth());
    dateToConvert.setDate(actualDate.getDate());
  
    const newTimestamp = dateToConvert.getTime();
    return newTimestamp;
  }
  ...
  getErrorMessage(errorCode: string): string {
    switch (errorCode) {
      ...
      case 'auth/user-not-found':
        return 'O usuário não corresponde a nenhuma credencial.';
      ...
      case 'auth/operation-not-allowed':
        return 'O tipo de conta correspondente a esta credencial ainda não está ativada.';
      ...
      case 'auth/operation-not-supported-in-this-environment':
        return 'Esta operação não é suportada no ambiente em que está sendo executada. Verifique se deve ser HTTP ou HTTPS.';
      case 'auth/timeout':
